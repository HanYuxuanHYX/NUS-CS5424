...
def new_order_transaction(w_id, d_id, c_id, num_items, item_num, supplier_warehouse, quantity):
    warehouse = Warehouse.objects.filter(W_ID=w_id)
    district = District.objects.filter(D_W_ID=w_id, D_ID=d_id)
    customer = Customer.objects.filter(C_W_ID=w_id, C_D_ID=d_id, C_ID=c_id)
    ...
    for i in range(num_items):
        item = Item.objects.filter(I_ID=item_num[i])
        stock = Stock.objects.filter(S_W_ID=w_id, S_I_ID=item_num[i])
    ...
    for i in range(num_items):
        item = Item.objects.filter(I_ID=item_num[i])
        stock = Stock.objects.filter(S_W_ID=w_id, S_I_ID=item_num[i])
        orderLine = OrderLine.objects.filter(OL_O_ID=n, OL_D_ID=d_id, OL_W_ID=w_id, OL_NUMBER=i)
...
if __name__ == '__main__':
    new_order_transaction(1279, 1, 1, 2, [68195, 26567], [1, 1], [1, 5])
